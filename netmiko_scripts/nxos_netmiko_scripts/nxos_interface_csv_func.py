...
time_now = datetime.datetime.now(tz=datetime.timezone.utc).isoformat(timespec='milliseconds')
...
# This function will return the interfaces operational configs and will save them into a csv file 
def get_interfaces(username, password, devices, port=22):
    ...
    for host in devices:
        ...
        try:
            ...
            # creata a csv file with the same name as the hostname/IP of the device connected with
            ouput_file =  f'{host}_{time_now}.csv' 
            with open(ouput_file, 'w', newline='') as f:
...
if __name__ == "__main__":
    ...
    # Get the interfaces output, please past the 'port=<kwargs>' if port is different than default 22
